...
namespace ToCBS
{
    internal class CBSBuilder
    {
        ...
        public static void BuildCBS(string[] vhdxs, string destination_path)
        {
            Dictionary<string, IList<CabinetFileInfo>> packages = BuildPackageListFromLiveFileSystem(vhdxs);
        ...
        private static List<PartitionInfo> GetPartitions(string[] vhdxs)
        {
            ...
            foreach (string vhdx in vhdxs)
            {
                ...
                if (hasOsPool)
                {
                    try
                    {
                        Disk msVirtualDisk = Vhd.Open(vhdx, true, null);
        ...
        private static Dictionary<string, IList<CabinetFileInfo>> BuildPackageListFromLiveFileSystem(string[] vhdxs)
        {
            Dictionary<string, IList<CabinetFileInfo>> packages = new();

            Console.WriteLine("GetPartitions: Start");
            List<PartitionInfo> partitions = GetPartitions(vhdxs);
