    ...
    public abstract class ArchiveInfo : FileSystemInfo
    {
            ...
            EventHandler<ArchiveProgressEventArgs> progressHandler)
        {
            ...
            ArchiveFileStreamContext streamContext =
                new(FullName, destDirectory, null)
                {
            ...
            EventHandler<ArchiveProgressEventArgs> progressHandler)
        {
            ...
            compressionEngine.Progress += progressHandler;
            ArchiveFileStreamContext streamContext =
                new(FullName, destDirectory, fileNames)
                {
                    EnableOffsetOpen = true
                };
            ...
            EventHandler<ArchiveProgressEventArgs> progressHandler)
        {
            ...
            ArchiveFileStreamContext streamContext = new(
            ...
            EventHandler<ArchiveProgressEventArgs> progressHandler)
        {
            ...
            using CompressionEngine compressionEngine = CreateCompressionEngine();
            compressionEngine.Progress += progressHandler;
            IDictionary<string, (Stream, FileAttributes, DateTime)> contextFiles =
                CreateStringDictionary(fileNames, sourceFileNames);
            ArchiveFileStreamContext2 streamContext = new(
                    FullName, sourceDirectory, contextFiles)
            {
                EnableOffsetOpen = true
            };
        ...
        /// the keys.</param>
        /// <returns>A filled dictionary of the strings.</returns>
        private static IDictionary<string, (Stream, FileAttributes, DateTime)> CreateStringDictionary(
            IList<string> keys, IList<(Stream, FileAttributes, DateTime)> values)
        {
            IDictionary<string, (Stream, FileAttributes, DateTime)> stringDict =
                new Dictionary<string, (Stream, FileAttributes, DateTime)>(StringComparer.OrdinalIgnoreCase);
            for (int i = 0; i < keys.Count; i++)
            {
                stringDict.Add(keys[i], values[i]);
            }
        ...
        private static void RecursiveGetRelativeFilePathsInDirectoryTree(
            string dir,
            string relativeDir,
            bool includeSubdirectories,
            IList<string> fileList)
